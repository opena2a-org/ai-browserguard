...
The detection/enforcement code runs in two realms: the **ISOLATED** content-script world (a separate JS context Chrome protects from the page) and a **MAIN**-world interceptor that wraps page globals (`window.open`, `HTMLFormElement.prototype.submit`, `history.pushState`, the `modify-dom` DOM-write sinks (`innerHTML`/`outerHTML`, `insertAdjacentHTML`, `setHTMLUnsafe`, `setAttribute`/`setAttributeNS`, `document.write`/`writeln`), an `Error.prepareStackTrace` trap, and — only while armed by an active delegation rule or the kill switch — `fetch`/`XHR`/`sendBeacon`, which install and tear down with delegation so the extension stays out of Chrome's failure-attribution chain for page-own requests when idle). The two communicate over a `MessageChannel` whose port is transferred once at startup (`src/content/interceptor.ts`, `src/content/index.ts`).
...
**Recommended hardening (tracked, not yet implemented):** add runtime schema validation (enum-constrain `agent.type`, bound string lengths) at the top of each background message handler so a forged-but-well-formed event cannot poison sessions, lifetime stats, or the opt-in contribute queue.

**Shipped -- CDP-layer enforcement (ADR-007, 0.7.0).** To remove the page-realm re-patch dependency for network egress, enforcement of site **`block` patterns** runs on the `chrome.debugger` (CDP) layer, which the page cannot re-patch (`src/background/cdp-enforcement.ts`). The `debugger` permission was already declared (used read-only for CDP-client *detection* via `chrome.debugger.getTargets()`); enforcement adds `chrome.debugger.attach()` plus the `Fetch` domain (`Fetch.requestPaused` -> `continueRequest`/`failRequest`). It is **opt-in** (`cdpEnforcementEnabled`, default off -- keeps the zero-runtime-banner posture by default; attaching shows Chrome's "started debugging this browser" bar), **per-tab**, and **attaches only when an agent is detected in the tab under an active delegation that carries at least one block pattern**, detaching on delegation expiry/revoke, kill switch, tab close, or setting-off. While attached, every request the CDP `Fetch` domain pauses on the tab -- including the vectors the page-realm deny leaves unwrapped: EventSource, element-`src`, Worker/iframe fetch, and navigation itself -- is blocked when it matches a block pattern and passes through otherwise; the CDP layer carries no agent-vs-user attribution, so the `network-request` *capability* withhold stays in the page realm (best-effort, attributed) and the CDP layer enforces only the deliberate, tab-wide domain blocks the user set. **WebSocket is the one exception and is not closed by this layer:** the `Fetch` domain does not pause `ws://`/`wss://` handshakes, and `Network.setBlockedURLs` blocking of WebSocket proved unreliable on the live extension's shared debugger session (it raced the handshake and leaked intermittently), so a WebSocket to a blocked domain still connects. This is measured and disclosed, not assumed; deterministic WebSocket blocking needs `declarativeNetRequest` (`resourceTypes: ['websocket']`, per-tab session rules), a new permission deferred to ADR-008 R2. The `npm run smoke:cdp` gate fires WebSocket every phase and asserts it stays open under enforcement, so this scope line cannot silently drift. On attach failure it **fails safe to the page-realm interceptor** -- the pre-0.7.0 shipping default -- never hard-blocking the user; a fail-open decision path means a policy bug can never hang a paused request or break the user's own browsing. Scope boundary, stated carefully: CDP enforcement exists for **in-page / injected agents** (the actors that re-patch globals). Against an **external** CDP framework the claim is narrower: on current Chrome, multi-client debugging means our attach typically *succeeds* alongside an external driver (measured on Chrome 145 with an active DevTools-protocol client, and with DevTools open), so blocked-domain egress is usually mediated on those tabs too -- but ABG does **not** rely on this against a browser-level adversary, which can detach, relaunch, or drive a different profile; for that population the defense remains detection, the ISOLATED `isTrusted`/CDP gates, posture guidance (ADR-008 R1), and the kill switch. `read-dom`/`screenshot`/`execute-script` enforcement is deferred to follow-up ADRs. The whole layer is exercised end-to-end against a real Chrome by `npm run smoke:cdp` (release gate): a receipt-logging fixture server proves each unwrapped vector is blocked only when enforcement is armed and flows again on every teardown path.
