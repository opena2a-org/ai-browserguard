...
**Decided direction -- CDP-layer enforcement (ADR-007).** To remove the page-realm re-patch dependency for the capabilities the MAIN world cannot guarantee, enforcement is being migrated onto the `chrome.debugger` (CDP) layer, which the page cannot re-patch. The `debugger` permission is already declared (used read-only today for CDP-client *detection* via `chrome.debugger.getTargets()`); enforcement adds `chrome.debugger.attach()` plus the `Fetch` domain. The design is **opt-in** (`cdpEnforcementEnabled`, default off -- keeps the zero-runtime-banner posture by default), **per-tab**, and **attaches only under an active delegation** for that tab, detaching on delegation expiry/revoke, kill switch, tab close, or setting-off. On attach failure (DevTools open, or an external CDP framework holds the tab's single debugger slot) it **fails safe to the page-realm interceptor** -- today's shipping default -- never hard-blocking the user. Scope boundary: CDP enforcement defeats the re-patch bypass for **in-page / injected agents** (the actors that re-patch globals); it does **not** stop an **external** CDP framework that owns the debugger slot -- for that population, detection plus the ISOLATED `isTrusted`/CDP gates remain the defense. First increment: off-realm enforcement of **site `block` patterns** for network egress via `Fetch.requestPaused` (also closing the WebSocket/EventSource/element-src/Worker vectors the page-realm deny left unwrapped, *for blocked domains*). The CDP layer carries no agent-vs-user attribution, so the `network-request` *capability* withhold stays in the page realm (best-effort, attributed) — the CDP layer enforces only the deliberate, tab-wide domain blocks. `read-dom`/`screenshot`/`execute-script` enforcement is deferred to follow-up ADRs.
